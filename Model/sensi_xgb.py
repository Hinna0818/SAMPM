...
def calculate_metrics_for_splits(dataset, test_sizes):
    ...
    for ratio in test_sizes:
        ...
        model = xgb.XGBClassifier(
            max_depth=9, min_child_weight=6, gamma=3, learning_rate=0.3417813803467016,
            subsample=0.820724210008386, colsample_bytree=0.5116227290149833,
            eval_metric='logloss', scale_pos_weight=2, max_delta_step=2
        )
...
plt.plot(test_sizes, auc, marker='o', label='AUROC')
...
# 保存图像
path = os.path.join(output_dir, "Sensitivity_AUROCC_F1_Brier_ACC.png")
